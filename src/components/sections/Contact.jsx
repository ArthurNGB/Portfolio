...
const AnimatedInputField = ({ value, onChange, placeholder, isLightMode, type = "text", rows, error, maxLength }) => {
  ...
  const inputClasses = `w-full border p-4 font-mono text-sm md:text-base font-bold tracking-wide focus:outline-none focus:border-samurai-gold transition-colors bg-transparent relative z-10 antialiased caret-[#D4AF37] cursor-text ${
    isLightMode
      ? "border-[#d4d4d4] text-neutral-900 focus:bg-white"
      : "border-neutral-800 text-neutral-200 focus:bg-white/5"
...
const Contact = ({ isLightMode, t, fadeUp }) => {
  ...
  const validate = () => {
    const e = {};
    if (!form.name.trim() || form.name.length > 50) e.name = t.errName;
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email) || form.email.length > 100) e.email = t.errEmail;
    if (!form.subject.trim() || form.subject.length > 70) e.subject = t.errSubject;
    if (form.message.trim().length < 20 || form.message.length > 4000) e.message = t.errMsg;
    return e;
  };
          ...
          <div className="grid lg:grid-cols-2 gap-16 lg:gap-12">
            ...
            <div className="space-y-10">
              ...
              <div className="flex gap-3">
                <a href="https://github.com/ArthurNGB" target="_blank" rel="noopener noreferrer" className={`flex items-center gap-2 border px-4 py-3 font-mono text-xs font-bold transition-all hover:border-samurai-gold hover:text-samurai-gold ${isLightMode ? "border-[#d4d4d4] text-neutral-600" : "border-neutral-800 text-neutral-400"}`}>
                  <Github size={16} /> GitHub
                </a>
                <a href="https://www.linkedin.com/in/arthur-nunes-2259772a3/" target="_blank" rel="noopener noreferrer" className={`flex items-center gap-2 border px-4 py-3 font-mono text-xs font-bold transition-all hover:border-samurai-gold hover:text-samurai-gold ${isLightMode ? "border-[#d4d4d4] text-neutral-600" : "border-neutral-800 text-neutral-400"}`}>
